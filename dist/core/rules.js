import { Rank } from '../types/index.js';
export var HandRank;
(function (HandRank) {
...
function evaluateFiveCards(cards) {
    ...
    if (isStraight && isFlush) {
        if (sorted[0].rank === Rank.Ace && sorted[1].rank === Rank.King)
            return { rank: HandRank.RoyalFlush, value: 0, cards: sorted };
    ...
    if (isFlush)
        return { rank: HandRank.Flush, value: getKickerValue(sorted), cards: sorted };
    if (isStraight)
        return { rank: HandRank.Straight, value: sorted[0].rank === Rank.Ace && sorted[1].rank === Rank.Five ? Rank.Five : sorted[0].rank, cards: sorted };
