...
import { generateDeck, shuffleDeck } from '../core/deck.js';
...
export class GameServer {
    ...
    handlePlayerAction(playerId, action) {
        ...
        switch (action.type) {
            ...
            case 'check':
                if (player.currentBet < this.state.currentMaxBet)
                    return; // Cannot check if there's a bet to call
            ...
            case 'raise':
                ...
                if (raiseAmount <= 0 || raiseTo <= this.state.currentMaxBet)
                    return;
    ...
    nextPhase() {
        ...
        if (this.state.players.find(p => p.id === this.state.currentTurnId).isFolded || this.state.players.find(p => p.id === this.state.currentTurnId).isAllIn) {
            ...
            if (active.length > 0)
                this.state.currentTurnId = active[0].id;
        }
        this.broadcastState();
    }
    determineWinners() {
    ...
    async checkAiTurn() {
        if (this.state.phase === GamePhase.GameOver || this.state.phase === GamePhase.Waiting)
            return;
